...
usage: qemu-system-i386 [options] [I<disk_image>]
...
=item B<-mem-path [path=]>I<path>B<[,prealloc=on|off][,share=on|off][,unlink=on|off]>
...
with a serial console.  Use B<C-a h> for help on switching between
...
certificates are specified with the VNC option B<x509>.
...
=item B<-netdev tap,id=>I<id>B<[,fd=>I<h>B<][,ifname=>I<name>B<][,script=>I<file>B<][,downscript=>I<dfile>B<][,helper=>I<helper>B<]>


=item B<-net tap[,vlan=>I<n>B<][,name=>I<name>B<][,fd=>I<h>B<][,ifname=>I<name>B<][,script=>I<file>B<][,downscript=>I<dfile>B<][,helper=>I<helper>B<]>
...
I<helper> to configure the TAP interface. The default network
helper executable is F</path/to/qemu-bridge-helper>.
...
are available use C<-clock help>.
...
Use C<-watchdog help> to list available hardware models.  Only one
...
Create an new object of type I<typename> setting properties
